          ...
          try {
            ...
            if (proposal) {
              proposalContext =
                ...
                ". Each item has its OWN boardKey — route per item. The proposal's top-level `botId` is the source-meeting id. If the user approves (e.g. 'confirm', 'yes', 'go', 'looks good', 'do it'), execute EACH item by its disposition: 'new' → board_create({boardKey: item.boardKey, title, kind, ownerEmail, sourceRef: <the proposal botId>, customerSlug: <lowercase hyphen-slug of item.company, e.g. 'AT&T'→'at-t'; omit if item.company is null>}); 'subtask' → board_create_subtask({parentId: targetId, title, kind, ownerEmail, sourceRef: <the proposal botId>, customerSlug: <same slug of item.company>}); 'update' → board_add_activity({id: targetId, kind:'logged_activity', body: note}). ALWAYS pass sourceRef = the proposal botId on creates so the task links back to its meeting recording/transcript. FIRST apply any corrections the user states (move an item to a different board, reassign owner, drop/add an item, change a disposition). Use board_list to avoid duplicating a card that already exists. After acting, reply with a concise grouped-by-board list of exactly what you created/updated. Create ONLY what is in the (corrected) proposal.]";
            }
