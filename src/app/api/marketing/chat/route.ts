...
function buildPrompt(opts: { messages: ChatMsg[]; viewer: string }): string {
  ...
  return [
    ...
    `• SEARCH DATA, the supermetrics MCP tools (mcp__supermetrics__*): Google Search Console for sc-domain:reddy.io, plus GA4, Google Ads, and LinkedIn Ads. GSC is the source of truth for what we rank for and what gets impressions; guardrail 10 below says how to use it. Queries are ASYNC (data_query submits, get_async_query_results polls), so fire them early and keep working while they run.`,
