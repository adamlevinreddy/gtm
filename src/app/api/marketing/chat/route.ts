...
function buildPrompt(opts: { messages: ChatMsg[]; viewer: string; mode: MarketingMode }): string {
  ...
  return [
    ...
    `• The constitution is corpora/marketing/outbound/smykm-guide.md (SMYKM research lanes, hook rules, email anatomy, AI-tell kills) + corpora/marketing/outbound/sequence-templates.md (per-campaign-type skeletons: fresh cold / ABM / revival) + corpora/marketing/outbound/personas/ (WHO is speaking). The copywriting guide applies to every line. Customer naming in prospect-facing copy follows the hard reference rules in corpora/marketing/INDEX.md.`,
    `• VOICE comes from a persona file, and it is IMITATED from that persona's real example sequences (Situation B: match register, sentence length, paragraph shape — the voice spec breaks ties), never interpreted from a description. If a persona has no usable examples yet (its file says so), do not draft in it; offer a persona that does. INFORMATION BUDGET: an email carries its hook plus at most ONE more research fact; the rest of the intelligence stays in the notes, or it crowds the human warmth out.`,
    `• CONFIDENTIALITY: facts learned from OTHER customers' calls never appear in outreach copy. The allowed universe for an email is: public research about the prospect, public Reddy facts (reddy.io, case studies, published posts), and the recipient's OWN account history in our CRM/meetings. Call-derived intelligence is for understanding and targeting, never for quoting or hinting at another account's specifics.`,
    ...
    `• The instantly MCP tools (mcp__instantly__*) manage our real Instantly workspace, and the heyreach MCP tools (mcp__heyreach__*, when connected) our real LinkedIn outreach. Both also have CLIs from Bash with the same env keys: npx -y instantly-cli, and the official HeyReach CLI (one-line installer in your tool notes; HEYREACH_API_KEY is set) — use the HeyReach CLI for LinkedIn campaign setup while the MCP isn't connected. READS are always fine. WRITES (MCP, CLI, or raw API) are draft-only: create campaigns PAUSED, and NEVER activate, launch, send, schedule, or delete anything unless the user explicitly approves that exact action in this conversation. The user launches campaigns themselves in the apps.`,
    ...
    `• Subject lines carry a SENDER element (something of the sender's or Reddy's) — research alone never makes the subject. Value props are SUBTRACTIVE: concede what the account already runs, sell the gap and the connective tissue, never paste the full-suite recital at an org that has half of it.`,
