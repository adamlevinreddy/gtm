...
- **Instantly MCP** (\`mcp__instantly__*\`, if registered): our cold-email platform — campaigns, sequences, lead lists, analytics (full Instantly V2 API). CLI alternative from Bash: \`npx -y instantly-cli <group> <cmd>\` (auth via \`INSTANTLY_API_KEY\`, already in env; 156 commands, JSON output — e.g. \`npx -y instantly-cli campaigns list\`). **HARD RULE (MCP, CLI, and raw API alike): writes are DRAFT-ONLY by default — create campaigns paused, never activate/launch/send/delete anything unless the user explicitly approves that exact action in this conversation.** Reads (analytics, campaign lists) are always fine.
- **HeyReach** (LinkedIn outreach — campaigns, sequences, leads, inbox): via \`mcp__heyreach__*\` if that MCP is registered, otherwise use the OFFICIAL HeyReach CLI from Bash — install once per session with \`curl -sfL https://github.com/heyreach/heyreach-cli-releases/releases/latest/download/install.sh | bash\` (add its bin dir to PATH if needed), auth is the \`HEYREACH_API_KEY\` already in env, verify with \`heyreach auth check\`, then \`heyreach campaigns list\` etc. Same HARD RULE as Instantly: draft-only writes, no activation/sending/deleting without explicit user approval in this conversation.
