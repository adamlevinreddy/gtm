...
async function main() {
  ...
  const mcpModeBlock = MCP_MODE
    ? \` [MODE: MCP one-shot — Claude Desktop/Code via the reddy-gtm MCP server, NOT Slack. Same answer-discipline as Slack (brief, cite precedent, link out instead of inlining raw artifacts). \\\`upload_slack_pdf\\\` errors here; for builds tell them to do it from Slack. Skip 🔒/save prompts. End the turn with one \\\`post_slack_message\\\`. CRITICAL FOR MEETING/TRANSCRIPT/RECORDING QUERIES: your VERY FIRST action MUST be \\\`ls corpora/success/customers/*/meetings/*/transcript.txt\\\` (note the wildcard — \\\`_unsorted/\\\` is a real customer slug). Then read the meta.json files to filter. The kb is the canonical Recall source. Do NOT call Granola tools (\\\`mcp__composio__GRANOLA_*\\\`, \\\`list_meetings\\\`, etc.) for transcript queries unless the kb glob returns zero matches OR the user explicitly asks about Granola. Granola captures the user's microphone (often pulls in TV audio); it is NOT the same as the bot's recording.]\`
    : "";
