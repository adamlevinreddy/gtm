...
const CLOSED_STATUSES: WorkItemStatus[] = ["done", "dismissed"];
function isOpen(s: WorkItemStatus): boolean {
...
export function effectiveHighPriority(item: WorkItem, now: Date = new Date()): boolean {
  ...
  if (item.dueAt && item.dueAt.getTime() - now.getTime() < WEEK_MS) return true;
  return false;
}
