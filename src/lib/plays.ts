...
` +
  ...
  outreach: {
    ...
    onCard: false,
    run: () =>
      `Help me build an outbound sequence. FIRST read the constitution: corpora/marketing/outbound/smykm-guide.md (research method + email anatomy), corpora/marketing/outbound/sequence-templates.md (per-campaign-type skeletons), corpora/marketing/outbound/personas/ (the voice personas), and corpora/marketing/copywriting-guide.md. The OUTREACH RULES in your instructions apply to everything below.
` +
      `ASK ME FIRST, in ONE short message (skip anything I already told you): (1) campaign type — fresh cold outbound, ABM on named accounts, or reviving people we've talked to before; (2) channels — Instantly email, HeyReach LinkedIn, or both; (3) the audience and where the list is — paste names here or upload a CSV/XLSX with the panel on this page (uploads land in corpora/marketing/uploads/); (4) WHICH PERSONA is speaking — offer the ones in personas/ with a suggestion based on the campaign type (never locked to it), flag any persona whose file says it isn't usable yet, and confirm identity details the file leaves open; (5) context — the offer or event, CTA, plus any authentic ties of the sender beyond the persona file's inventory (anything true that could connect to these prospects); (6) what we already know about these accounts' stack and situation (tools they run, prior conversations) so the value prop can subtract instead of recite; (7) any must-hit points or constraints. Then STOP and wait.
` +
...
` +
      `4) PERSONALIZE: fill each prospect's merge fields ({{smykmSubject}}, {{smykmOpener}}, {{hookCallback}}, and whatever the skeleton needs). VOICE IS IMITATED, not interpreted: the persona file's real examples are the writing sample (copywriting constitution Situation B — match their register, sentence length, paragraph shape); the voice spec breaks ties. INFORMATION BUDGET: each email carries the hook plus at most ONE more research fact — the rest stays in the notes table; proof placement follows the persona spec. Subjects follow the sender-element rule (one element must be the persona's/Reddy's — research alone never makes the subject). Value props are SUBTRACTIVE: concede what the account already has, pitch the gap and the connective tissue. Where the research surfaced no in-window hook, SAY SO and use the company/space fallback (or the space-lingo move) rather than faking depth.
` +
